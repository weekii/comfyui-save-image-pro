...
# Try to set locale, fallback to safe defaults if system locale is not supported
try:
    original_locale = locale.setlocale(locale.LC_TIME, '')
except locale.Error:
    # Try common fallback locales
    fallback_locales = ['C.utf8', 'en_US.utf8', 'C', 'POSIX']
    original_locale = None
    for loc in fallback_locales:
        try:
            original_locale = locale.setlocale(locale.LC_TIME, loc)
            print(f"[save_image_extended] Using fallback locale: {loc}")
            break
        except locale.Error:
            continue

    if original_locale is None:
        print("[save_image_extended] Warning: Could not set any locale, using system default")
        original_locale = 'C'
